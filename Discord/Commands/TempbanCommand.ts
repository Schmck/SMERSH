...
export const TempbanCommand: Command = {
    ...
    autocomplete: async (client: Client, interaction: AutocompleteInteraction): Promise<void> => {
        ...
        if (players) {
            ...
            const filtered = choices.filter(choice => choice.name && typeof(choice.name) === 'string' && choice.name.toLowerCase().startsWith(focusedValue.value.toLowerCase()) || choice.name.toLowerCase().includes(focusedValue.value.toLowerCase()))
            if (filtered.length) {
