...
export const RetardCommand: Command = {
    name: "retard",
    description: "in game command to let another player know just how retarded you think they are",
    ...
    run: async (client: Client, interaction: CommandInteraction) => {
        ...
        if (player) {
            const message = `You went full retard ${player.Name} never go full retard`
            ...
            if (playa) {
                const message = `You went full retard ${playa.Playername} never go full retard`
                const chatUrl = env["BASE_URL"] + ChatRoute.PostChat.Action
                const chatUrlencoded = `ajax=1&message=${message}&teamsay=-1`
                await axios.post(chatUrl, chatUrlencoded, config)
                await interaction.followUp({
