...
export const TempbanCommand: Command = {
    ...
    run: async (commandBus: CommandBus, name: string, id: string, reason: string, duration: string) => {
        ...
        if (duration && duration && typeof (duration) === 'string') {
            ...
            if (player) {
                await commandBus.execute(new ApplyPolicyCommand(Guid.create(), player.Id, env["COMMAND_CHANNEL_ID"], Action.Ban, player.Name, reason, new Date(), unbanDate))

                const message = `${player.Name} was banned for ${untilString} for ${reason} until ${unbanDate.toString().split(' GMT')[0]}`
