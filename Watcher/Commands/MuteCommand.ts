...
export const MuteCommand: Command = {
    ...
    run: async (commandBus: CommandBus, caller: PlayerSearchReport, player: PlayerInfo, name: string, id: string, reason: string, duration: string) => {
        ...
        if (duration && duration && typeof (duration) === 'string') {
            ...
            if (player) {
                await commandBus.execute(new ApplyPolicyCommand(Guid.create(), player.Id, env["COMMAND_CHANNEL_ID"], Action.Mute, player.Playername, reason, caller.Name, new Date(), unbanDate))

                if (caller && caller.Invisible) {
                    return;
                } 

                const message = `${player.Playername} was muted for ${untilString} for ${reason ? reason : 'no reason'} until ${unbanDate.toString().split(' GMT')[0]}`
