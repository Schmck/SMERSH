...
export class ApplyPolicyCommandHandler implements ICommandHandler<ApplyPolicyCommand> {
    ...
    async execute(command: ApplyPolicyCommand) {
        const { Id, PlayerId, ChannelId, Action, Name, Reason, BanDate, UnbanDate, PlainId  } = command
        ...
        await domain.applyPolicy(Id, ChannelId, Action, Name, Reason, BanDate, UnbanDate, PlainId);
        await domain.commit()
        return;
